...
from . import models
from .forms import user_entry, user_datas, StudInfoForm
...
class InfoAddView(CreateView):
    form_class = StudInfoForm
    template_name = 'home/enroll.html'
...
#         return Student.objects.get(pk=self.request.GET.get('pk'))  # or request.POST
...
class Enrollment(View):
    def get(self, request):
        ...
        try:
            student = Student.objects.get(name__id=int(uid))
        except Student.DoesNotExist:
            student = None
        if student:
            form = StudInfoForm(instance=student)
            return render(request, 'home/enroll.html', {'form': form})
        form = StudInfoForm()
        # form.name = user
        return render(request, 'home/enroll.html', {'form': form})
    ...
    def post(self, request):
        uid = request.session.get('user_id', None)
        form = StudInfoForm(request.POST, request.FILES, initial={'name': User.objects.get(id=int(uid))})
        # print('-------------------')
        # print(form.data)
        # print('-------------------')
        message = '请检查填写的内容'
        if form.is_valid():
            same_id_user = Student.objects.filter(idCard=form.cleaned_data['idCard'])
            # if same_id_user:  # 身份证号已存在
            #
            #     message = '该身份证号已存在，请检查！'
            #     print(message)
            #     return render(request, 'home/enroll.html', {'form': form, 'message': message})
            # print('-------------------')
            # print(form.data)
            # print('-------------------')
            obj = form.save(commit=False)
            obj.name = User.objects.get(id=int(uid))
            obj.save()
...
class InfoEdit(View):
    def get(self, request, pk):
        ...
        form = StudInfoForm(instance=obj)
        # action = 'info_edit'
        return render(request, 'home/student_update_form.html', {'form': form})

    def post(self, request, pk):
        obj = Student.objects.get(id=pk)
        form = StudInfoForm(request.POST, request.FILES, instance=obj)
        print(form.instance)
        print('-------------------')
        print(form.data)
        print('-------------------')
        ...
        if form.is_valid():
            ...
            # if same_id_user:  # 身份证号已存在
            #     message = '该身份证号已存在，请检查！'
            #     return render(request, 'home/student_update_form.html', {'form': form, 'message': message})
            print('-------------------')
            print(form.data)
            print('-------------------')
...
class CertificationPrintView(View):
    def get(self, request):
        uid = request.session.get('user_id', None)
        obj = Student.objects.get(name__id=int(uid))

        return render(request, 'home/print.html', {'student': obj})

# def enroll(request):
#     if request.method == 'POST':
#         form = StudInfoForm(request.POST, request.FILES)
#         if form.is_valid():
#             form.save(commit=True)
#
